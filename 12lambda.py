...
람다(lambda) 함수
    ...
    사용해서 간편하게 작성 할 수 있고, 재사용되지 않는 1회성 함수를
    만들떄 사용한다.
'''
# 두수의 합을 일반적인 함수로 정의
def two_sum(x, y):
...
# list_data 의 원소 갯수만큼 람다식을 반복 호출하여 얻어진 결과를 리스트로 반환한다.
...
형식]
    실행문1 if 조건식 else 실행문2
        ...
        람다식에서 조건부 표현식을 사용할떄는 :을 붙히지 않는다.
